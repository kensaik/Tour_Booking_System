# Báo cáo Tuần 2

**Tuần:** 2 (09/03/2026 - 15/03/2026)
**Nhóm:** 14
**Đề tài:** Hệ Thống Quản Lý Tour Du Lịch
**Nhóm trưởng:** Lê Duy Mạnh - 2351010124

## 1. Công việc đã hoàn thành

| Thành viên            | MSSV         | Công việc                                                              | Link Commit/PR |
| Lê Duy Mạnh           | [2351010124] | Review Use case, tổng hợp tài liệu, vẽ sơ đồ ERD                    |                |
| Nguyễn Trần Minh Quân | [2351010175] | Viết Use case phân hệ Du khách, bắt đầu vẽ Wireframe                  |                |
| Trương Hưng Phát      | [2351010154] | Viết Use case phân hệ Công ty Du lịch                                   |                |

## 2. Tiến độ tổng thể

| Hạng mục                                    | Trạng thái | % |
| Khởi động (Chọn đề tài, Repo setup)         | Done         | 100% |
| Phân tích yêu cầu (Use case, Wireframe)     | In Progress   | 60% |
| Thiết kế kiến trúc (DB, API)                | Chưa bắt đầu | 0% |
| Backend API                                 | Chưa bắt đầu | 0% |
| Frontend UI                                 | Chưa bắt đầu | 0% |
| Testing                                     | Chưa bắt đầu | 0% |

**Tổng tiến độ: 20%**

## 3. Kế hoạch tuần tới (Tuần 3)

| Thành viên | Công việc dự kiến |
| Lê Duy Mạnh | Hoàn thiện ERD, review database schema |
| Nguyễn Trần Minh Quân | Hoàn thiện Wireframe tất cả màn hình |
| Trương Hưng Phát | Thiết kế API endpoints, viết API docs |

## 4. Khó khăn / Cần hỗ trợ
- [ ] Cần thống nhất về naming convention cho database
- [ ] Cần quyết định cách xử lý thanh toán (mock hay tích hợp thật)

*Ngày nộp: 15/03/2026*
*Xác nhận của Nhóm trưởng: Lê Duy Mạnh
