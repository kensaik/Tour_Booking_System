# Báo cáo Tuần 8

**Tuần:** 8 (20/04/2026 - 26/04/2026)
**Nhóm:** 14
**Đề tài:** Hệ Thống Quản Lý Tour Du Lịch
**Nhóm trưởng:** Lê Duy Mạnh - 2351010124

## 1. Công việc đã hoàn thành

| Thành viên            | MSSV         | Công việc                                                              | Link Commit/PR |
| Lê Duy Mạnh           | [2351010124] | Final regression testing, compile test report                          |                |
| Nguyễn Trần Minh Quân | [2351010175] | Final bug fixes, polish UI/UX, prepare demo                            |                |
| Trương Hưng Phát      | [2351010154] | Hoàn thành test report, test summary, test data documentation          |                |
| Tô Nguyễn Sơn Nam     | [2351050109] | Performance testing, final E2E test run, demo setup                     |                |

## 2. Tiến độ tổng thể

| Hạng mục                                    | Trạng thái | % |
| Khởi động (Chọn đề tài, Repo setup)         | Done         | 100% |
| Phân tích yêu cầu (Use case, Wireframe)     | Done         | 100% |
| Thiết kế kiến trúc (DB, API)                | Done         | 100% |
| Backend API                                 | Done         | 100% |
| Frontend UI                                 | Done         | 100% |
| Testing                                     | Done         | 100% |

**Tổng tiến độ: 100%**

## 3. Kết quả Testing

### Test Coverage Summary

| Loại Test | Số Test Cases | Passed | Failed | Skipped |
|------------|---------------|--------|--------|---------|
| Unit Tests | - | - | - | - |
| Integration Tests | - | - | - | - |
| E2E Tests | - | - | - | - |
| Manual Tests | - | - | - | - |

### Bugs Found & Fixed

| Severity | Found | Fixed | Remaining |
|----------|-------|-------|-----------|
| Critical | - | - | 0 |
| High | - | - | 0 |
| Medium | - | - | 0 |
| Low | - | - | 0 |

## 4. Demo & Presentation

- [ ] Demo video recorded
- [ ] Presentation slides prepared
- [ ] Final documentation completed

## 5. Khó khăn / Cần hỗ trợ
- [x] Đã hoàn thành tất cả các tính năng theo yêu cầu
- [x] Đã sẵn sàng cho demo

*Ngày nộp: 26/04/2026*
*Xác nhận của Nhóm trưởng: Lê Duy Mạnh*
