...
def register():
    ...
    if role not in [UserRole.COMPANY, UserRole.GUEST]:
        return jsonify(
            error="Bad Request", message="Invalid role. Must be COMPANY or GUEST"
        ), 400
    ...
    if role == UserRole.COMPANY:
        ...
        if not company_name:
            return jsonify(
                error="Bad Request", message="Company name is required for COMPANY role"
            ), 400
        ...
        if not full_name:
            return jsonify(
                error="Bad Request", message="Full name is required for GUEST role"
            ), 400
