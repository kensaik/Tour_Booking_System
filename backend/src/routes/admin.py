...
def create_destination():
    ...
    if Destination.query.filter_by(name=name).first():
        return jsonify(error="Bad Request", message="Destination name already exists"), 400

    new_dest = Destination(name=name, description=description)
    db.session.add(new_dest)
    db.session.commit()

    return jsonify(message="Destination created successfully", destination={"id": new_dest.id, "name": new_dest.name}), 201
...
def update_destination(id):
    ...
    db.session.commit()
    return jsonify(message="Destination updated successfully", destination={"id": destination.id, "name": destination.name}), 200
...
def delete_destination(id):
    ...
    if destination.tours.count() > 0:
        return jsonify(error="Bad Request", message="Cannot delete destination because it is linked to existing tours"), 400
...
def update_commission(id):
    ...
    if new_rate < 0 or new_rate > 100:
        return jsonify(error="Bad Request", message="Commission rate must be between 0 and 100"), 400

    company.commission_rate = float(new_rate)
    db.session.commit()
    return jsonify(message="Commission rate updated successfully", commission_rate=company.commission_rate), 200
