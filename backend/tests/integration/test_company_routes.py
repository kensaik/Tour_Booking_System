...
def test_add_departure_success(client, auth_client):
    co = make_company(approved=True)
    tour = make_tour(co)
    start = (datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(days=10)).isoformat()
    end = (datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(days=13)).isoformat()
    res = auth_client(co).post(
...
def test_add_departure_start_after_end_returns_400(client, auth_client):
    co = make_company(approved=True)
    tour = make_tour(co)
    start = (datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(days=15)).isoformat()
    end = (datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(days=10)).isoformat()
    res = auth_client(co).post(
        f"/api/company/tours/{tour.id}/departures",
