...
export default function AdminCompaniesPage() {
  ...
  const updateCommissionMutation = useMutation({
    mutationFn: ({ id, rate }: { id: any, rate: number }) => AdminService.updateCommission(id, rate),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-companies'] })
      setModalType(null)
    }
  ...
  if (isLoading) return <AdminLayout><div className="text-center py-20">Đang tải danh sách công ty...</div></AdminLayout>
  if (error) return <AdminLayout><div className="text-center py-20 text-red-500">Lỗi tải danh sách.</div></AdminLayout>
    ...
    <AdminLayout>
      ...
      <div className="bg-surface-container-lowest rounded-xl p-4 shadow-sm border border-outline-variant mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div>
            <label htmlFor="filter-status" className="sr-only">Trạng thái</label>
            <select 
              id="filter-status" 
              value={statusFilter}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-surface-container border-b border-outline-variant">
              <tr>
                <th className="text-left px-6 py-4 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Công ty</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Liên hệ</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Chiết khấu</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Trạng thái</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Ngày tham gia</th>
            ...
            <tbody className="divide-y divide-outline-variant">
              {filteredCompanies.map((company: any) => (
                <tr key={company.id} className={`hover:bg-surface-container-low transition-colors ${!company.is_active ? 'opacity-60 bg-surface-container-lowest' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <StatusBadge status={company.is_approved ? 'approved' : 'pending'} type="company" />
                      {!company.is_active && <span className="text-[10px] font-bold text-error uppercase">Bị khóa</span>}
                    </div>
                  </td>
      ...
      >
        <form onSubmit={(e) => {
          ...
          <div>
            <label className="block text-sm font-medium text-on-surface mb-1">Email đăng nhập</label>
            <input 
              type="email" 
              required
              className="w-full px-4 py-2 rounded-lg border border-outline-variant bg-surface-container-lowest"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant font-medium">%</span>
            </div>
            <p className="mt-2 text-xs text-on-surface-variant italic">
              * Tỷ lệ phần trăm doanh thu hệ thống sẽ thu trên mỗi đơn đặt tour thành công của công ty này.
            </p>
          </div>

          <div className="pt-4">
            <button 
              onClick={() => updateCommissionMutation.mutate({ id: selectedCompany.id, rate: formData.commission_rate })}
        ...
        {selectedCompany && (
          <div className="space-y-6">
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <StatusBadge status={selectedCompany.is_approved ? 'approved' : 'pending'} type="company" />
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-surface-container">
                ...
                <div>
                  <p className="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Email</p>
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg bg-surface-container">
                ...
                <div>
                  <p className="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Ngày gia nhập</p>
                  <p className="text-sm font-medium text-on-surface">{formatDate(selectedCompany.created_at)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 rounded-lg bg-surface-container">
                <User className="w-5 h-5 text-primary" />
                <div>
                  <p className="text-xs text-on-surface-variant uppercase font-bold tracking-wider">ID Hệ thống</p>
