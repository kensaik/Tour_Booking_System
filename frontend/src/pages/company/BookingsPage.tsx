...
export default function CompanyBookingsPage() {
  ...
  const filteredBookings = bookings.filter(booking => {
    ...
    const matchesPayment = paymentFilter === 'Tất cả thanh toán' ||
                          (paymentFilter === 'Đã thanh toán' && booking.payment_status === 'fully_paid') ||
                          (paymentFilter === 'Chưa thanh toán' && (booking.payment_status === 'unpaid' || booking.payment_status === 'pending'))
                ...
                <tr key={booking.id} className="hover:bg-surface-container-low transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div>
                      ...
                      <p className="text-sm text-on-surface-variant">{booking.guest_phone || 'N/A'}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-on-surface">
                    {booking.departure ? formatDate(booking.departure.start_date) : 'N/A'}
                  </td>
                  <td className="px-6 py-4 text-on-surface text-center">
                    {booking.num_people}
                  </td>
        ...
        {viewingBooking && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-on-surface-variant mb-1 font-semibold uppercase text-[10px]">Khách hàng</p>
                <p className="font-bold text-base">{viewingBooking.guest_name}</p>
                <p className="">{viewingBooking.guest_phone || 'N/A'}</p>
              </div>
              <div>
                <p className="text-on-surface-variant mb-1 font-semibold uppercase text-[10px]">Tour</p>
                <p className="font-bold text-base line-clamp-1">{viewingBooking.tour?.name}</p>
                <p className="">{viewingBooking.departure ? formatDate(viewingBooking.departure.start_date) : 'N/A'}</p>
              </div>
            </div>
            ...
            <div className="bg-surface-container-low p-5 rounded-2xl space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatPrice(viewingBooking.total_price / viewingBooking.num_people)}</span>
              </div>
            ...
            <div>
              <p className="text-on-surface-variant mb-3 font-semibold uppercase text-[10px]">Trạng thái hệ thống</p>
              <div className="flex gap-6">
                ...
                <div className="flex-1">
                  ...
                  <StatusBadge status={viewingBooking.payment_status} type="payment" showIcon={false} />
                </div>
              </div>
            </div>
